...
MemPool CreateMemPool(const size_t size)
{
    ...
    else
    {
        ...
        uint8_t *const restrict buf = (uint8_t*)malloc(size*sizeof *buf);
...
void *MemPoolRealloc(MemPool *const restrict mempool, void *const ptr, const size_t size)
{
    ...
    else
    {
        ...
        uint8_t *const resized_block = (uint8_t*)MemPoolAlloc(mempool, size);
...
ObjPool CreateObjPool(const size_t objsize, const size_t len)
{
    ...
    else
    {
        ...
        uint8_t *const restrict buf = (uint8_t*)calloc(len, aligned_size);
...
BiStack CreateBiStack(const size_t len)
{
    ...
    uint8_t *const buf = (uint8_t*)malloc(len*sizeof *buf);
