...
uniform sampler2D worldTexture; // The texture of the game's world/collision map.
...
void main()
{
	...
	vec2 world = textureSize(worldTexture, 0);
	...
	vec2 xyRay = vec2((coord.y * rayTexSize.x) + coord.x, TAU * light.z);
	...
	for(float v = step(xyRay.x,xyRay.y), d = 0.; d < MAXRADIUS * v; d++)
		...
		*/
		if (light.z < d + light.z * texture2D(worldTexture, (light.xy + (xyRay = delta * d)) * world).a) break;
